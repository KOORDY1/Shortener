...
def upgrade() -> None:
    if "video_drafts" not in _table_names():
        op.create_table(
            "video_drafts",
            ...
            sa.Column(
                "script_draft_id",
                sa.String(length=36),
                sa.ForeignKey("script_drafts.id", ondelete="CASCADE"),
                nullable=False,
            ),
            ...
            sa.Column(
                "render_config", sa.JSON(), nullable=False, server_default=sa.text("'{}'::json")
            ),
            sa.Column(
                "timeline_json", sa.JSON(), nullable=False, server_default=sa.text("'{}'::json")
            ),
    ...
    idx_vd = _index_names("video_drafts")
    if "ix_video_drafts_candidate_id" not in idx_vd:
        op.create_index("ix_video_drafts_candidate_id", "video_drafts", ["candidate_id"])
    if "ix_video_drafts_script_draft_id" not in idx_vd:
        op.create_index("ix_video_drafts_script_draft_id", "video_drafts", ["script_draft_id"])

    if "exports" not in _table_names():
        op.create_table(
            "exports",
            sa.Column("id", sa.String(length=36), primary_key=True),
            sa.Column(
                "video_draft_id",
                sa.String(length=36),
                sa.ForeignKey("video_drafts.id", ondelete="CASCADE"),
                nullable=False,
            ),
            sa.Column("status", sa.String(length=32), nullable=False),
    ...
    idx_ex = _index_names("exports")
    if "ix_exports_video_draft_id" not in idx_ex:
