...
| 우선순위 | 항목 | 구현 내용 |
|----------|------|----------|
| ~~**1**~~ ✅ | ~~`created_seq` 동시성 안전 확보~~ | retry-on-conflict 루프 (최대 3회). unique 제약 위반 시 rollback → 재시도. 소진 시 409 응답. |
| ~~**2**~~ ✅ | ~~피드백 후 stale 데이터 해소~~ | `onStatusChange` 콜백에 `router.refresh()` 연결. 피드백 제출 후 서버 컴포넌트가 최신 데이터로 리렌더링. |
| ~~**3**~~ ✅ | ~~후보 목록 카드에 selected/tags 표시~~ | `candidate-card.tsx`에 "채택됨" 배지 + failure_tags 한글 뱃지 표시. |
| ~~**4**~~ ✅ | ~~metadata 타입 강화~~ | `dict[str, Any]` → `FeedbackMetadata` Pydantic 모델 (`new_rank: int | None` 등). 프론트도 `FeedbackRequestMetadata` / `FeedbackResponseMetadata` 타입 분리. |
| ~~**5**~~ ✅ | ~~`_parse_snapshot` 방어 코딩~~ | `try/except (ValueError, TypeError)` 래핑. 잘못된 값 시 기본값 반환. |
...
| ~~운영자 피드백 로그 스키마~~ | ✅ 완료 | `CandidateFeedback` DB 모델, 마이그레이션 0005, POST/GET API, 프론트엔드 패널 |
| ~~길이 정책 재검토~~ | ✅ 완료 | `LengthPolicy` 3계층 (search window / core span / render target), config.py 환경변수 |
| ScoringWeights 튜닝: golden set 기반 가중치 조정 실험 | 미구현 | golden set 데이터 축적 후 실증 필요 |
...
*작성 기준: 2026-03-31 / 본문 기준 커밋 `7b55688`*
