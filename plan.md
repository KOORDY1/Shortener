...
> **다음 우선순위 (후보 품질 개선 중심)**
>
> 1. **오프라인 평가셋 정교화** — 실 드라마 에피소드로 golden set 구축, Recall@K 기준 현황 측정
> 2. **실패 유형 분류 체계 정립** — 맥락 결여 / payoff 없음 / 중복 상위 노출 등 taxonomy 수동 태깅
> 3. **후보 다양성 강화** — 현재 IOU/Jaccard dedupe에서 인물·사건·payoff 유형 기반 diversity-aware selection으로 전환
> 4. **길이 정책 재검토** — search window · core span · render target 분리 기준 정비 및 실증
> 5. **운영자 피드백 로그 스키마 도입** — 선택/탈락/수정 이력을 DB에 기록, 향후 pairwise ranking 데이터 기반으로 연결
...
**정량 지표 (현재 측정 기반 도입 예정):**

| 지표 | 설명 | 현황 |
|------|------|------|
| Recall@K | 운영자가 채택한 후보가 상위 K개 안에 있는 비율 | golden set 미구축, 측정 미실시 |
| 채택율 | 생성된 후보 중 운영자가 실제 선택한 비율 | 피드백 로그 미존재 |
| 후보 다양성 | 상위 K개 후보의 인물·사건·payoff 유형 분산 | 현재 측정 체계 없음 |
...
│  │    → 컷 밀도·반응샷 시드         │
│  └─ Track C: generate_audio_seeds_live() │
│       → ebur128 기본 / astats 폴백  │
│          / librosa optional 보정    │
│                                     │
...
| 오프라인 평가 스크립트 | `scripts/evaluate_candidates.py` | **implemented, local/eval only** | Recall@K, 점수 분포, 타임라인 커버리지 |
| Vision 재랭크 v2 (GPT-4.1) | `vision_candidate_refinement.py` | **experimental** | `VISION_CANDIDATE_RERANK=true` + API 키; 비용 발생 |
| LLM Arc Judge | `candidate_rerank.py` | **experimental** | `LLM_ARC_JUDGE_ENABLED=true`; gpt-4.1-mini, 비용 발생 |
...
- **Track A (대화):** 드라마의 핵심은 대사 아크. 대화 밀도·QA 구조가 가장 직접적인 쇼츠 신호이며, 자막이 있는 에피소드에서 가장 신뢰도 높은 시그널이다.
- **Track B (시각):** 컷 전환·반응샷 = 편집자가 주목하는 비언어 감정 신호. 자막 없는 구간에서도 독립 탐색 가능하다.
- **Track C (오디오):** 음량 에너지 스파이크 = 클라이맥스/전환점 위치 추정. 대사 내용과 독립적으로 피크 위치를 포착한다.
- 세 트랙은 서로 독립 신호이므로 병렬 탐색 후 통합 스코어로 경쟁한다. 한 트랙이 약하더라도 다른 트랙이 보완한다.

### 5.2 왜 Recall → Precision 2단계인가

- **1단계 (후보 생성):** 넓게 잡아서 놓치지 않음. 최대 14개 후보를 heuristic scoring으로 생성한다.
- **2단계 (Vision·LLM rerank):** 비용이 비싼 모델을 상위 소수에만 적용한다 (gating). Vision은 상위 8개, LLM Arc Judge는 상위 5개에만 적용된다.
- **왜 전체를 바로 LLM에 넣지 않는가:** 1에피소드당 수백 개의 candidate window가 생성되며, 각 window에 GPT-4V를 호출하면 분당 수천 토큰의 비용이 발생한다. 따라서 heuristic scoring으로 상위 K개로 먼저 좁힌 뒤 Vision/LLM을 적용하는 2단계 gating 구조를 사용한다.
...
- 하지만 composite는 과연결/과설명 위험이 있다 (§6 실패 유형 참조).

### 5.4 현재 가설의 한계

- 언어 모델 없이 키워드·임베딩·arc heuristic으로 approximation. 의미 이해는 없다.
- 운영자 피드백 없이 scoring weight는 주관적 초기값이다. 현재 default 가중치가 최적인지 알 수 없다.
- "좋은 쇼츠"의 정의가 아직 장르별로 다를 수 있다. 현재는 단일 기준으로 평가한다.
...
| 유사한 후보 여러 개 상위 노출 | IOU NMS + Jaccard dedupe만 적용 | IOU 0.52 임계값, Jaccard 토큰 dedupe | **미해결**: 인물·사건·payoff 유형 기반 다양성 보장 없음 |
...
- **오프라인 평가셋:** 스크립트 기반 합성 영상(18초)으로 smoke test만 통과 확인. 실 드라마 에피소드 기반 golden set 미구축.
- **Recall@K 계산 스크립트** (`evaluate_candidates.py`)는 구현되어 있으나, 실제 운영자 채택 이력이 없어 의미 있는 측정이 불가능한 상태.
- **운영자 피드백 로그 스키마** 미존재. 선택/탈락/수정 이력이 DB에 기록되지 않는다.

### 6.3 현재 핵심 병목

1. **golden set 부재** → 품질 개선이 실제로 효과 있는지 측정 불가
2. **scoring weight 근거 부족** → 현재 default 가중치가 최적인지 알 수 없음
3. **다양성 보장 없음** → 상위 후보가 비슷한 장면에 쏠릴 수 있음
...
| 항목 | 후보 품질 영향도 | 비용 | 구현 난이도 | 의존성 |
|------|----------------|------|------------|--------|
| 오프라인 평가셋 정교화: 실 드라마 에피소드 2~3편으로 golden set 구축, `evaluate_candidates.py`로 Recall@K 측정 | 높음 (측정 기반 마련) | 낮음 (수작업) | 낮음 | 에피소드 샘플 확보 |
| 실패 유형 taxonomy 수동 태깅: 최근 생성 후보 50~100개를 §6.1 유형으로 분류 | 높음 (개선 방향 명확화) | 낮음 (수작업) | 낮음 | golden set |

### 60~90일 (단기)

| 항목 | 후보 품질 영향도 | 비용 | 구현 난이도 | 의존성 |
|------|----------------|------|------------|--------|
| diversity-aware selection: 인물·사건·payoff 유형 기반 dedupe 추가 | 중~높음 | 낮음 | 중간 | taxonomy 체계 |
| 운영자 피드백 로그 스키마: DB에 선택/탈락/수정 이력 기록 | 중간 (장기 데이터 수집) | 낮음 | 낮음 | DB 마이그레이션 |
| 길이 정책 실증: search window와 render target 분리 기준 재검토 | 중간 | 낮음 | 낮음 | golden set |
| ScoringWeights 튜닝: golden set 기반 가중치 조정 실험 | 높음 | 낮음 | 중간 | golden set + Recall@K |

### 90일+ (이후)

- **pairwise ranking 데이터 생성:** 피드백 로그 → (후보 A vs 후보 B) 선호 쌍 → LLM-based reranker 또는 경량 ranking 모델 학습 준비. 향후 방향, 현재 미구현.
- **장르별 scoring profile 확장:** drama_conflict, emotional_release, awkward_comedy 같은 장르별 프로파일로 확장 가능한 축으로 설계됨 — 단, 현재 미구현. 현재는 default/reaction_heavy/payoff_heavy 3개 프로파일만 존재.
- **단위 테스트 / 통합 테스트 보강:** tone_signals·QA 스코어·Arc 탐색·IOU dedupe 단위 테스트 4개 케이스; 실 SRT + 영상 기반 E2E 통합 테스트.
- **Speaker Diarization:** pyannote.audio 의존성 평가 후 도입 결정. 현재 out of scope.

### 향후 검토 (배포 자동화)

- **YouTube 자동 업로드:** 채널 할당량(~6건/일) 확인 후 도입 결정. 현재 out of scope.
...
최대 14개 Candidate. **현재 한계:** 인물·사건·payoff 유형 기반 다양성 보장 없음. 향후 diversity-aware selection으로 전환 예정.
...
**호출 조건:** `LLM_ARC_JUDGE_ENABLED=True`. 상위 `LLM_ARC_JUDGE_TOP_K`(기본 5)개에 gpt-4.1-mini 적용. arc_closed(bool), standalone(0-10), shorts_fit(0-10), adjustment([-1.0, 1.0]), reason 반환.
...
- **운영자 피드백 로그 스키마** (선택/탈락/수정 이력 DB 기록) — 현재 미구현. 향후 pairwise ranking 데이터 기반으로 연결 예정.
- **golden set 기반 정량 평가** — 실 드라마 에피소드 2~3편 필요. 향후 구축 예정.
...
| `LLM_ARC_JUDGE_ENABLED` | `false` | gpt-4.1-mini Arc Judge (비용 발생) |
...
*작성 기준: 2026-03-31 / 본문 기준 커밋 `f5becda`*
